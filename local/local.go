...
func New(password [5]*core.Password, listenAddr, remoteAddr *net.TCPAddr) *LsLocal {
	return &LsLocal{
...
func (local *LsLocal) handleConn(userConn *net.TCPConn) {
	...
	go func() {
		err := local.DecodeCopy(userConn, proxyServer, *pm)
		if err != nil {
	...
	local.LocalEncodeCopy(proxyServer, userConn, pm)
}
